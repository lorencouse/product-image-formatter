...
class ImageResizerGUI:
    ...
    def download_images(self, sku):
        api_url = f"https://wholesale.williams-trading.com/rest/product-images/{sku}?format=xml"  # Update the API URL
        ...
        if response.status_code == 200:
            ...
            for image_elem in root.findall(".//image_large_url"):
                ...
                print("Image URL:", image_url)  # Print each URL for debugging
            ...
            for index, image_url in enumerate(image_urls):
                ...
                if image_response.status_code == 200:
                    ...
                    image_path = os.path.join(sku_folder, image_filename)
    ...
    def download_and_process_images(self):
        ...
        if not output_name:  # If output_name is blank, fetch the name from the API
            name_api_url = (
        ...
        if sku and output_name:
            ...
            output_folder = os.path.join(
                self.output_folder.get(), output_name
            )  # Modify this line
            os.makedirs(output_folder, exist_ok=True)

            sku_folder = os.path.join(self.output_folder.get(), sku)
            image_files = [
                file for file in os.listdir(sku_folder) if file.endswith(".jpg")
            ]

            self.input_images.extend(
                os.path.join(sku_folder, image_file) for image_file in image_files
            )
            self.update_selected_images_listbox()
